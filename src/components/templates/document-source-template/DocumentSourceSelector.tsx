...
function DocumentSourceSelector({
  ...
  const handleQuestionToggle = (questionId: string) => {
    ...
    const existingIds = new Set(
      selectedSystemQuestions.map((q: any) => q.id.toString())
    );

    if (existingIds.has(questionId)) {
      // Nếu câu hỏi đã được chọn trước đó, xóa khỏi danh sách
      const updatedQuestions = selectedSystemQuestions.filter(
        (q: any) => q.id.toString() !== questionId
      );
  ...
  const handleSelectAllQuestions = () => {
    ...
    const newQuestions = questions.data.filter(
      (q: any) => !existingIds.has(q.id.toString())
    );
  ...
  const handleDeselectAllQuestions = () => {
    ...
    const currentPageQuestionIds = new Set(
      questions.data.map((q: any) => q.id.toString())
    );
    const updatedQuestions = selectedSystemQuestions.filter(
      (q: any) => !currentPageQuestionIds.has(q.id.toString())
    );
              ...
              <div className="p-4 w-full">
                    ...
                    {questions?.data?.length > 0 && (
                      <>
                        ...
                        {questions.data.some((q: any) =>
                          selectedSystemQuestions.some(
                            (sq: any) => sq.id.toString() === q.id.toString()
                          )
                        ) && (
                          <Button
                            variant="outline"
                ...
                <div className="space-y-5">
                  {questions?.data?.map((question: any, idx: number) => {
                    const isSelected = selectedSystemQuestions.some(
                      (q: any) => q.id.toString() === question.id.toString()
                    );
                      ...
                      <div key={question.id} className="relative">
                        <div className="flex items-start gap-3">
                          <Checkbox
                            ...
                            checked={isSelected}
                            onCheckedChange={() =>
                              handleQuestionToggle(question.id.toString())
                            }
                            className="mt-2"
                          />
