...
function MatrixTemplatesPage() {
  ...
  // Use default templates for now (replace with API call later)
  const { data: templatesData, isLoading } = useMatrixTemplatesService();
  const { mutate: deleteTemplate } = useDeleteMatrixTemplateService();

  // For now, use default templates. In production, this would come from API
  const templates = templatesData?.data?.content || DEFAULT_MATRIX_TEMPLATES;

  // Filter templates based on search term
  const filteredTemplates = templates.filter((template: MatrixTemplateConfig) =>
  ...
  const handleDeleteTemplate = (templateId: string) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa template này?")) {
      // For now, just show toast. In production, call API
      toast.success("Template đã được xóa thành công!");
      console.log("Deleting template:", templateId);
    }
  };

  const handleDuplicateTemplate = (template: MatrixTemplateConfig) => {
    ...
    // For now, just show toast. In production, call API to create new template
    toast.success("Template đã được sao chép! Chuyển đến trang chỉnh sửa...");
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold font-calsans">Quản lý Template Ma Trận</h1>
            <p className="text-gray-600 font-questrial">
              Tạo và quản lý các template ma trận đề thi
            </p>
          </div>
            ...
            <CardContent>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">{template.parts.length}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">Tổng mức độ:</span>
                  <span className="font-medium">
                    {template.parts.reduce((total, part) => total + part.difficultyLevels.length, 0)}
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-1">
                  {template.parts.map((part) => (
                    <div key={part.id} className="flex items-center gap-2 text-xs">
                      <div className={`w-3 h-3 rounded ${part.color}`}></div>
                      <span>{part.name}</span>
                      {part.label && (
      ...
      {filteredTemplates.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            {searchTerm ? "Thử tìm kiếm với từ khóa khác" : "Chưa có template nào được tạo"}
          </p>
          <Link href="/staff/matrix-templates/create">
            <Button>
              <Plus className="w-4 h-4 mr-2" />
